...
def calcu_hyperedge(cord, n_neigh=10, metric='minkowski'):
    '''
    构建空间超边 (Spatial Hyperedge)。
    逻辑：以每个点为中心，将其与其 K 个最近邻居圈在一起，形成一条超边。
    返回：hyperedge_index [2, num_entries]
         Row 0: Node index (属于该超边的节点)
         Row 1: Hyperedge index (超边ID，即中心点ID)
    '''
    # 包含自身，所以 k = n_neigh + 1
    ...
    # 构建超边索引
    # neigh_index[i] 包含了属于超边 i (中心点 i) 的所有节点
    
    # 超边 ID (即中心点 i)，每个点重复 k 次
    hyperedge_ids = np.repeat(np.arange(num_nodes), n_neigh+1)
    
    # 节点 ID (超边内的成员)
    node_ids = neigh_index.ravel()
    
    # 堆叠成 [2, E]
    hyperedge_index = np.vstack((node_ids, hyperedge_ids))
...
def hgat_data(adata, 
    ...
    for batch_tmp in batch_order:
        ...
        # 2. 新增：计算超边 (Hyperedges)
        hyper_tmp = calcu_hyperedge(coord_tmp, n_neigh=n_neigh_hom, metric='minkowski')
        
        feat_dict[batch_tmp] = feat_tmp
        coord_dict[batch_tmp] = coord_tmp
        adj_dict[batch_tmp, '0', batch_tmp] = adj_tmp
        hyperedge_dict[batch_tmp] = hyper_tmp # 存储超边
